...
int main(int argc, char *argv[])
{
    ...
    if (isWebInterface) {
        ...
        while (1) {
            ...
            if (buffer[4] == 0x2F && buffer[5] == 0x73 && buffer[6] == 0x74 && buffer[7] == 0x79) { // Forward slash and sty
                write(newSocket, HTTP_OK, sizeof(HTTP_OK) - 1);
                write(newSocket, CONTENT_TYPE_CSS, sizeof(CONTENT_TYPE_CSS) - 1);
                write_all(newSocket, style, sizeof(style));
            }

            if (buffer[4] == 0x2F && buffer[5] == 0x73 && buffer[6] == 0x63 && buffer[7] == 0x72) { // Forward slash and scr
                write(newSocket, HTTP_OK, sizeof(HTTP_OK) - 1);
